...
function umrohweb_customize_register( $wp_customize ) {
    ...
    // ... (Kode Customizer lainnya diasumsikan tetap ada/sama) ...
}
...
function add_travel_meta_boxes() {
    // Data Travel
    add_meta_box('travel_details', '1. Kontak & Info Travel', 'render_travel_details_meta_box', 'travel', 'normal', 'high');
    add_meta_box('travel_banners', '2. Banner Carousel', 'render_travel_banner_meta_box', 'travel', 'normal', 'high');
...
function render_travel_details_meta_box($post) {
    ...
    ?>
    <p><label><strong>Nomor WhatsApp (Wajib):</strong></label><br>
    <input type="text" name="travel_phone" value="<?php echo esc_attr($phone); ?>" placeholder="628123456789" style="width:100%;"></p>
    <p><label><strong>Alamat Kantor:</strong></label><br>
    <textarea name="travel_address" style="width:100%;" rows="2"><?php echo esc_textarea($address); ?></textarea></p>
    <p><label><strong>Link Google Maps (Embed):</strong></label><br>
    <input type="text" name="travel_maps" value="<?php echo esc_attr($maps); ?>" style="width:100%;"></p>
    <?php
}
...
function render_travel_banner_meta_box($post) {
    ...
    <p><input type="text" name="travel_banner_1" value="<?php echo esc_url($b1); ?>" placeholder="URL Banner 1" style="width:100%; margin-bottom:5px;"></p>
    <p><input type="text" name="travel_banner_2" value="<?php echo esc_url($b2); ?>" placeholder="URL Banner 2" style="width:100%; margin-bottom:5px;"></p>
    <p><input type="text" name="travel_banner_3" value="<?php echo esc_url($b3); ?>" placeholder="URL Banner 3" style="width:100%;"></p>
    <?php
}
...
function render_travel_extras_meta_box($post) {
    ...
    function addTesti() { document.getElementById('testi-wrapper').insertAdjacentHTML('beforeend', '<div style="margin-bottom:10px; border-bottom:1px solid #eee; padding-bottom:10px;"><input type="text" name="testi_name[]" placeholder="Nama" style="width:100%; margin-bottom:5px;"><textarea name="testi_text[]" placeholder="Isi" style="width:100%;"></textarea></div>'); }
    </script>
    <?php
}

// --- RENDER FORM PAKET (NEW RICH DATA) ---
...
function render_package_details_meta_box($post) {
    ...
    <div style="display:grid; grid-template-columns: 1fr 1fr; gap: 15px;">
        <p>
            <label><strong>Harga (Teks):</strong></label><br>
            <input type="text" name="package_price" value="<?php echo esc_attr($price); ?>" placeholder="Rp 25.5 Juta" style="width:100%;">
        </p>
        <p>
            <label><strong>Durasi (Hari):</strong></label><br>
            <input type="text" name="package_duration" value="<?php echo esc_attr($duration); ?>" placeholder="9 Hari" style="width:100%;">
        </p>
        <p>
            <label><strong>Maskapai:</strong></label><br>
            <input type="text" name="package_airline" value="<?php echo esc_attr($airline); ?>" placeholder="Saudi Airlines" style="width:100%;">
        </p>
        <p>
            <label><strong>Hotel Bintang:</strong></label><br>
        ...
        <p>
            <label><strong>Bulan/Tanggal Keberangkatan:</strong></label><br>
            <input type="text" name="package_date" value="<?php echo esc_attr($depart_date); ?>" placeholder="12 Oktober 2024" style="width:100%;">
        </p>
    </div>
    <?php
}
...
function save_travel_custom_meta($post_id) {
    // Travel Meta
    $fields = ['_travel_phone', '_travel_address', '_travel_maps', '_travel_banner_1', '_travel_banner_2', '_travel_banner_3'];
    ...
    if (isset($_POST['testi_name'])) {
        $testis = []; foreach($_POST['testi_name'] as $i => $n) { if($n) $testis[] = ['name'=>sanitize_text_field($n), 'text'=>sanitize_textarea_field($_POST['testi_text'][$i])]; }
